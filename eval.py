...
from graph import graph as _graph, build_initial_state, RECURSION_LIMIT
...
def _timeout_sentinel(row_args: tuple, *, error: str | None = None) -> dict:
    ...
    rec = {
        ...
        "total_retry_count": 0, "val_retry_count": 0, "deploy_backstop_count": 0, "deploy_retry_count": 0,
        "routing_log": [], "resource_match": {},
        "first_val_pass_s": None, "total_elapsed_s": 0,
    }
    if error:
        rec["error"] = error[:300]
    return rec
...
# ── Chạy 1 row ────────────────────────────────────────────────────────────────
def _run_row(idx, difficulty, prompt, gt_types) -> dict:
    ...
    first_val_pass_s = None      # wall-time tới lần A4 (validation) PASS đầu tiên
    code_at_plan_valid = ""      # snapshot code lúc A4 pass đầu (TRƯỚC deploy-repair)
    val_attempts_at_pv = 0       # số vòng val tới khi plan-valid lần đầu
    err = None
    try:
        for chunk in _graph.stream(state, config={"recursion_limit": RECURSION_LIMIT},
                                   stream_mode="updates"):
            ...
            for node, update in chunk.items():
                ...
                # Agent đang chạy (giữ trong 5 tên). Fail-còn-retry KHÔNG đánh terminal —
                # node kế (retry) hoặc cuối hàm mới chốt pass/fail.
                if node == "validation":
                    last_val_fb = update.get("fix_feedback") or last_val_fb
                    if first_val_pass_s is None and (update.get("fix_feedback") or {}).get("overall_passed"):
                        first_val_pass_s = round(time.time() - t_start, 1)
                        # Snapshot code A4 vừa validate (TRƯỚC mọi deploy-repair) → đo plan-stage fair.
                        code_at_plan_valid = final_state.get("generated_code", "")
                        val_attempts_at_pv = final_state.get("total_val_attempts", 0)
                    _set(idx, "Deployment")
                elif node in _NEXT:
                    _set(idx, _NEXT[node])
    except Exception as e:
    ...
    _set(idx, "", "pass" if bool((dep or {}).get("ok")) else "fail", total_s)
    ...
    return {
        ...
        "resource_match": cmp,
        "first_val_pass_s": first_val_pass_s,   # mốc: tới khi A4 pass lần đầu (None nếu chưa từng pass)
        "code_at_plan_valid": code_at_plan_valid,   # snapshot code A4-pass (plan-stage, trước deploy)
        "val_attempts_at_plan_valid": val_attempts_at_pv,
        # resource_match plan-stage: declared types của snapshot (KHÔNG dùng resources_created
        # đã deploy) → so fair với baseline plan-only.
        "resource_match_plan": (_resource_comparison(gt_types, _declared_types(code_at_plan_valid))
                                if (gt_types is not None and code_at_plan_valid.strip()) else {}),
...
def _plan_record(r: dict) -> dict:
    """Projection plan-stage (A4-pass) → schema plan-only như baseline (so fair).
    Dùng code_at_plan_valid + resource_match_plan (declared types), KHÔNG deploy."""
    code = r.get("code_at_plan_valid") or ""
    ok = r.get("first_val_pass_s") is not None and bool(code.strip())
    va = r.get("val_attempts_at_plan_valid", 0)
    return {
        "row": r["row"], "difficulty": r.get("difficulty"), "prompt": r.get("prompt"),
        "architecture": {"ok": bool(code.strip())},
        "security": {},
        "engineering": {"ok": bool(code.strip()), "generated_code": code},
        "validation": {"ok": ok, "plan_ok": ok, "validate_ok": ok},
        "deployment": None,
        "total_retry_count": va,
        "val_retry_count": va,
        "resource_match": r.get("resource_match_plan") or {},
        "total_elapsed_s": r.get("first_val_pass_s") or 0,
    }
...
def main():
    ...
    ap.add_argument("--plan-out", default=None,
                    help="Ghi THÊM file plan-stage (snapshot code lúc A4-pass, KHÔNG deploy) "
                         "→ so fair với baseline plan-only. Không tốn LLM thêm.")
    args = ap.parse_args()
    ...
    print(f"eval.py | dataset={Path(args.dataset).name} | rows={len(rows)} | "
          f"workers={args.workers}{shard_s}{' | resume' if args.resume else ''}\n")
        ...
        if args.workers <= 1:
            for ra in rows:
                try:
                    ...
                    _record(_timeout_sentinel(ra, error=str(e)))
        else:
    ...
    if args.plan_out:
        plan_recs = [_plan_record(r) for r in results]
        _atomic_write(Path(args.plan_out), plan_recs)
        n_pv = sum(1 for r in plan_recs if r["validation"]["plan_ok"])
        print(f"[plan-stage] {n_pv}/{len(plan_recs)} plan-valid → saved {args.plan_out}")
    _print_summary(results, out_path)


def _print_summary(results: list[dict], out_path: Path) -> None:
    n = len(results)
    ...
    print(f"  {'-' * 36}")
    # Mẫu số cascade theo pipeline; Deployment lấy tổng case (giống evaluate.py).
    ...
    # Mean thời gian (bỏ qua None): valid = tới A4 pass đầu, total = cả run.
    fv = [r["first_val_pass_s"] for r in results if r.get("first_val_pass_s") is not None]
    tt = [r["total_elapsed_s"] for r in results if r.get("total_elapsed_s")]
    if fv:
        print(f"  {'-' * 36}")
        print(f"  avg_time_valid  {sum(fv) / len(fv):.1f}s   [tới A4 pass đầu]")
    if tt:
        print(f"  avg_time_total  {sum(tt) / len(tt):.1f}s   [cả run]")
    print(f"{'=' * 50}")
    print(f"{ok['deployment']}/{n} pass → saved {out_path}")
